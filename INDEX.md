...
- src/resgraph/evals/ — the analyst's eval harness (D24): deterministic graders, pinned judge, store-isolated runner, report, judge spend breaker (D29a), provider adapter for a pluggable worker (D29c); datasets + runs live in evals/; ground-up explainer in docs/evals-explained.md
- BENCHMARKS.md — measured numbers with hardware + method (D4)
- EVALS.md — the working set of the iteration log (D24, D34): protocol
  rules, the paid-run ledger, open registrations; the model-fed slice is
  context-core-marked and fingerprinted
- EVALS-HISTORY.md — the closed record, moved verbatim (D34, append-only,
  never fed); byte-exact snapshots in docs/evals-archive/, procedure in
  docs/evals-compaction-runbook.md
...
- src/resgraph/gateway/ — serving gateway (D30–D33, D40–D43): precedence router
  with recorded source, endpoint registry (one alias, many endpoints;
  capability admission; `GET /v1/models`), caller/operator contract + billing (accounts, wallet, `GET /v1/usage`), dispatch policy
