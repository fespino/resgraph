...
into training data or gets overfit by prompt iteration, and a
score that drops sharply under a re-skin means the agent was
reading the generator's templates, not the graph. The decay
pressure itself is measured, in a different arena: Anthropic's
[AI-resistant technical evaluations](https://www.anthropic.com/engineering/AI-resistant-technical-evaluations)
is about the optimization take-home they use in *hiring*, where
candidates may use AI — it stopped discriminating as models
improved, one model generation cutting its solution from 2,164
cycles to 1,487, level with the best of a thousand-plus human
candidates, and their recovery was novelty: task shapes outside
the training distribution. A hiring exercise and an agent eval age
by the same mechanism — the models keep getting better at anything
typical — but the defenses differ by failure: re-skins answer
leakage and template overfit, while a scenario type that saturates
from capability growth alone has expired, and its replacement
comes from the generator's parameter space, not from fresh paint.

And the scenarios are generated by code, not by an LLM:
Microsoft's
[STATE-Bench](https://github.com/microsoft/STATE-Bench) discloses
LLM-synthesized tasks as its scaling trade-off, which is the
disclosed version of a real alternative — but a synthesized
task's ground truth is only as reliable as the model that wrote it, and this
