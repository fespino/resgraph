...
failures made the 92% *drop* to 83%, because one of those hits had
been subsidized by an invalid path all along. None of this came from
reading transcripts and feeling uneasy; every number came off
...
committed before the agent ran once.
...
measures how fast evals erode as models improve (their internal
benchmark's median solve went from 2,164 cycles to 1,487 across one
model generation) and reaches for novelty as the recovery — re-skins
...
this iteration a
regression. It was the eval working: when honesty goes up and
accuracy goes down, the accuracy was borrowing against the honesty.
...
Closing that gap took six more pre-registered iterations, three of
