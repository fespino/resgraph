...
| worked example (contrastive pair: quiet window abstained, deep cause committed) | PREFIX | system block 1, after the output contract | teaches the discrimination rules could not (EVALS.md iterations 3–6; a single example taught a bias); illustrative ids are excluded from referential citability — the harness seeds citable ids from post-breakpoint blocks and tool results only |
