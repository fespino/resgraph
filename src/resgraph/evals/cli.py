...
def routing_table(
    specs: list[str],
    task_class: str = "judgment",
    out: str = "evals/routing-quality.yaml",
) -> None:
    ...
    for spec in specs:
        if "=" not in spec:
            raise typer.BadParameter(f"expected alias=run_file, got {spec!r}")
        alias, path = spec.split("=", 1)
        rows = [json.loads(line) for line in Path(path).read_text().splitlines() if line.strip()]
        summary = arm_summary(alias, rows)
        run_id = rows[0].get("run_id", Path(path).stem)
        date = f"{run_id[:4]}-{run_id[4:6]}-{run_id[6:8]}" if len(run_id) >= 8 else run_id
        entry: dict[str, Any] = {}
        for key, (field, places) in ROUTING_INPUTS.items():
            value = summary[key]
            entry[field] = (
                round(value, places) if places is not None and value is not None else value
            )
        scores[alias] = entry | {"run": path, "date": date}
    doc = {"scores": {task_class: scores}}
    header = "# GENERATED by resgraph-evals routing-table -- do not hand-edit.\n"
    Path(out).write_text(header + yaml.safe_dump(doc, sort_keys=True))
    typer.echo(f"{len(scores)} arms -> {out} (task_class={task_class})")
