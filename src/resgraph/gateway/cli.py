...
def serve(
    ...
    health for unpriced backends only — cadence is per-setup data
    (`probe_interval_s` in models.yaml, default 60s). Keep-alive is set
    for streams that hold connections through long generations. The
    fall-forward budget caps the money the failure path may spend per
    UTC day before refusing with the reason stated."""
    uvicorn.run(
        create_app(
